...
namespace FinancialServices.Api.Endpoints.V1.Transaction
{
    ...
    public class GetConsolidatedDataEndpoint : IEndpoint
    {
        ...
        {
            return routeBuilder.MapGet("/{date}", (HttpContext httpContext, [FromServices] IGetConsolidatedReportUseCase getConsolidatedReportUseCase, [FromRoute] DateTime date) =>
            {
                try
                {

                    var r  = getConsolidatedReportUseCase.GetConsolidatedReport(date.ToUniversalTime().Date);
